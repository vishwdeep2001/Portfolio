...
const Navbar = () => {
  ...
  return (
    <nav className='paddingindX w-full flex items-center py-5 top-0 z-20 bg-primary'>
      <div className='w-full flex justify-between items-center max-w-7xl mx-auto'>
        <Link to="/" className='flex items-center gap-2' onClick={() => {
          ...
          <p className='text-white text-18 font-bold cursor-pointer flex'>Vishwdeep &nbsp; <span className='sm:block hidden'> | Singharia</span></p>
        </Link>
        <ul className='list-none hidden sm:flex flex-row gap-10'>
          {navLinks.map((link) => (
            <li key={link.id} className={`${active === link.title ? 'text-white' : 'text-secondary'} hover:text-white text-18 font-medium cursor-pointer`} onClick={() => setActive(link.title)}>
